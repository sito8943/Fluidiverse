export default class AtomicBot {
    ...
    Next = (p, i = undefined) => {
        if (!i)
            this.currentI = this.InnerStateLinks(p.Name, this.currentI.Name);
        else
            this.currentI = this.InnerStateLinks(p.Name, i.Name);
    ...
    Action = (i = undefined) => {
        this.currentA = this.A.filter((item) => {
            if (!i) if (item.InnerState === this.currentI) return item;
            else if (item.InnerState === i) return item;
        })[0]
